...
# plot mean inference rating by utterance, prior, and qud ----
...
# relevel prior
means$prior <- factor(means$prior, levels = c("higher","lower"))
...
ggplot(means, aes(x=utterance, y=Mean)) +
  #geom_violin(data = d, aes(x=utterance, y=response), scale="width",color="gray80", fill = "gray80") +
