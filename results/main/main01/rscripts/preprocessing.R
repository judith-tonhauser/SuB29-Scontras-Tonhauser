...
# age and gender of recruited participants
table(d$age) #18-82
length(which(is.na(d$age))) # 3 missing values
# exclude outliers (0, 3330) before calculating mean
mean(d$age,na.rm=TRUE) #43
...
# qud
d$qud = case_when(grepl("Sue says", d$stimulus) ~ "ai",
                      grepl("partner tells", d$stimulus) ~ "nai",
                  TRUE ~ "error")
table(d$qud)
...
length(unique(d$participantID)) #4 participants excluded
...
length(unique(d$participantID)) #873 so 6 participants excluded
...
tmp = d %>%
  ...
  tally
tmp
nrow(tmp) # 40 (everything was combined)
min(tmp$n) #10
mean(tmp$n) #21.8
max(tmp$n) #34
